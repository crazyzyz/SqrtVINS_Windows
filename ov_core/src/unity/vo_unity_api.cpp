...
VOErrorCode vo_initialize(const VOCameraParams* camera_params,
    ...
    try {
        return ov_core::VOUnityBridge::getInstance().initialize(
            *camera_params, tracking_params);
    } catch (...) {
        return VO_ERROR_INVALID_PARAM;
    }
}
...
VOErrorCode vo_process_frame(const unsigned char* image_data,
...
VOErrorCode vo_get_features(VOFeature* features, int max_count, int* out_count) {
...
void vo_get_version(int* major, int* minor, int* patch) {
    if (major) *major = VO_VERSION_MAJOR;
    if (minor) *minor = VO_VERSION_MINOR;
    if (patch) *patch = VO_VERSION_PATCH;
}
