 ...
 * the yaml file!!! This limits things quite a bit, but simplified the below
 * implementation.
 *
 * NOTE: The camera and imu have nested, but those are handled externally.
 * They first read the "imu0" or "cam1" level, after-which all values are at the
 * same level.
 */
class YamlParser {
public:
...
private:
  ...
  void parse(const cv::FileNode &file_node, const std::string &node_name,
    ...
    try {
      ...
      if (required) {
        ...
        PRINT_DEBUG("unable to parse %s node of type [%s] in the config file "
                    "(not required)\n",
                    node_name.c_str(), typeid(node_result).name());
      }
    }
  ...
  void parse_external_yaml(const std::string &external_node_name,
    ...
    try {
      ...
      PRINT_WARNING(YELLOW "unable to parse %s node of type [%s] in [%s] in "
                           "the external %s config file!\n" RESET,
                    node_name.c_str(), typeid(node_result).name(),
