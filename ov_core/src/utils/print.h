...
// Windows compatibility - undefine ERROR macro from wingdi.h
#ifdef _WIN32
#ifdef ERROR
#undef ERROR
#endif
...
public:
  ...
  enum PrintLevel {
    ...
    INFO = 2,
    WARNING = 3,
    ERROR_LVL = 4,  // Renamed from ERROR to avoid Windows wingdi.h macro conflict
    SILENT = 5
  };
...
#define PRINT_ERROR(...)                                                       \
  ov_core::Printer::debugPrint(ov_core::Printer::PrintLevel::ERROR_LVL,        \
                               __FILE__, TOSTRING(__LINE__), __VA_ARGS__);
#else
#define PRINT_ALL(x...)                                                        \
  ov_core::Printer::debugPrint(ov_core::Printer::PrintLevel::ALL, __FILE__,    \
