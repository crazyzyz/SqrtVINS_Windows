...
int main(int argc, char **argv) {
  ...
  PRINT_DEBUG("ransac threshold: %f\n", ransac_th);
  ...
  for (int i = 0; i < 2; i++) {
    Eigen::Matrix<DataType, 8, 1> cam0_calib;
  ...
  extractor =
      new TrackKLT(cameras, num_pts, num_aruco, !use_stereo, method,
                   fast_threshold, grid_x, grid_y, min_px_dist, ransac_th);
