...
Simulator::Simulator(VioManagerOptions &params_) {
  ...
  while (true) {
    ...
    // Check if it fails
    if (!success_pose) {
      PRINT_ERROR(RED "[SIM]: unable to find jolt in the groundtruth data to "
                      "initialize at\n" RESET);
      std::exit(EXIT_FAILURE);
    }
  ...
  for (int i = 0; i < params.state_options.num_cameras; i++) {
    ...
    while (true) {
      ...
      std::vector<std::pair<size_t, Eigen::Vector2f>> uvs =
...
void Simulator::perturb_parameters(std::mt19937 gen_state,
  ...
  for (int i = 0; i < params_.state_options.num_cameras; i++) {
    ...
    MatX intrinsics = params_.camera_intrinsics.at(i)->get_value();
    for (int r = 0; r < 4; r++) {
...
bool Simulator::get_state(double desired_time,
  ...
  Vec3 true_bg_interp = (1 - lambda) * hist_true_bias_gyro.at(id_loc) +
                        lambda * hist_true_bias_gyro.at(id_loc + 1);
  Vec3 true_ba_interp = (1 - lambda) * hist_true_bias_accel.at(id_loc) +
                        lambda * hist_true_bias_accel.at(id_loc + 1);

  // Finally lets create the current state
  imustate(0, 0) = desired_time;
  imustate.block(1, 0, 4, 1) = rot_2_quat(R_GtoI).cast<double>();
  imustate.block(5, 0, 3, 1) = p_IinG.cast<double>();
  imustate.block(8, 0, 3, 1) = v_IinG.cast<double>();
  imustate.block(11, 0, 3, 1) = true_bg_interp.cast<double>();
  imustate.block(14, 0, 3, 1) = true_ba_interp.cast<double>();
  return true;
}

bool Simulator::get_next_imu(double &time_imu, Vec3 &wm, Vec3 &am) {
  ...
  // Current state values
  Mat3 R_GtoI;
  Vec3 p_IinG, w_IinI, v_IinG, alpha_IinI, a_IinG;
  ...
  Vec3 omega_inI = w_IinI;
  Vec3 gravity;
  gravity << 0.0, 0.0, params.gravity_mag;
  Vec3 accel_inI = R_GtoI * (a_IinG + gravity);
...
Simulator::project_pointcloud(const Mat3 &R_GtoI, const Vec3 &p_IinG, int camid,
                              const std::unordered_map<size_t, Vec3> &feats) {
  ...
  Vec3 p_IinC = params.camera_extrinsics.at(camid).block(4, 0, 3, 1);
  ...
  for (const auto &feat : feats) {
    ...
    Vec3 p_FinI = R_GtoI * (feat.second - p_IinG);
    Vec3 p_FinC = R_ItoC * p_FinI + p_IinC;
    ...
    uv_dist = camera->distort(uv_norm.cast<DataType>()).cast<float>();
...
void Simulator::generate_points(const Mat3 &R_GtoI, const Vec3 &p_IinG,
                                int camid,
                                std::unordered_map<size_t, Vec3> &feats,
                                int numpts) {
  ...
  for (int i = 0; i < numpts; i++) {
    ...
    cv::Point2f uv_norm;
    uv_norm = camera->undistort(uv_dist);
    ...
    Vec3 p_FinI = R_ItoC.transpose() * (p_FinC - p_IinC);
    Vec3 p_FinG = R_GtoI.transpose() * p_FinI + p_IinG;
