...
int main(int argc, char **argv) {
  ...
  std::vector<Vec3> vec_am;
  std::vector<Vec3> vec_wm;
  std::vector<std::vector<std::vector<std::pair<size_t, Eigen::Vector2f>>>>
      vec_feats;
    ...
    if (hascam) {
      ...
      for (size_t camid = 0; camid < feats.size(); camid++) {
        for (size_t i = 0; i < feats.at(camid).size(); i++) {
          assert(feats.at(camid).at(i).first ==
                 vec_feats.at(ct_cam).at(camid).at(i).first);
          assert(feats.at(camid).at(i).second(0) ==
                 vec_feats.at(ct_cam).at(camid).at(i).second(0));
          assert(feats.at(camid).at(i).second(1) ==
                 vec_feats.at(ct_cam).at(camid).at(i).second(1));
        }
      }
      ct_cam++;
    }
